...
class Penjualan(models.Model):
    ...
    detailpenjualan_ids = fields.One2many(
                            comodel_name='tesproject.detailpenjualan',
    ...
    def _compute_totalbayar(self):
        for rec in self:
            a = sum(self.env['tesproject.detailpenjualan'].search([('penjualan_id','=',rec.id)]).mapped('subtotal'))
            rec.total_bayar = a
    ...
    def _ondelete_penjualan(self):
        if self.detailpenjualan_ids:
            a=[]
            for rec in self:
                a = self.env['tesproject.detailpenjualan'].search([('penjualan_id','=',rec.id)])
                print(a)
                for ob in a:
...
# DEF WRITE TRIGGER SAAT EDIT
    def write(self,vals):
        ...
        for rec in self:
            b = self.env['tesproject.detailpenjualan'].search([('penjualan_id','=',rec.id)])
            print(a)
            print(b)
            for databaru in b:
                if databaru in a:
...
class DetailPenjualan(models.Model):
    _name = 'tesproject.detailpenjualan'
    _description = 'New Description'

    name = fields.Char(string='Nama')
    penjualan_id = fields.Many2one(comodel_name='tesproject.penjualan', string='Detail Penjualan')
    barang_id = fields.Many2one(comodel_name='tesproject.barang', string='List Barang')
    ...
    def create(self,vals):
        ...
        if record.qty:
            self.env['tesproject.barang'].search([('id','=',record.barang_id.id)]).write({'stok' : record.barang_id.stok-record.qty})
            return record
